...
import WorkspaceNextSteps from "./components/WorkspaceNextSteps";
    ...
    <div className="min-h-screen pb-16">
      <header className="border-b border-slate-300 bg-white px-6 py-5">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            <h1 className="text-xl font-bold text-slate-900">{data.project.name}</h1>
            <p className="mt-0.5 text-slate-600">
              {data.project.address ?? `${data.project.commercial_area} / ${data.project.official_neighborhood}`}, {data.project.city}
            </p>
          </div>
        ...
        {data.project.demo_location_assumption && (
          <p className="mt-2 inline-block rounded bg-slate-100 px-2 py-1 text-xs text-slate-500">
            {data.project.location_note ?? "מיקום הדגמה – הכתובת לא סופקה על ידי החברה"}
          </p>
        )}
      </header>

      <section className="grid grid-cols-2 gap-4 px-6 py-5 sm:grid-cols-4">
        <Kpi label="סה״כ יחידות" value={String(data.project.total_units)} />
        <Kpi label="יחידות סטנדרטיות שתומחרו" value={String(data.project.standard_units_priced)} />
        <Kpi label="יחידות מיוחדות שנותחו פרטנית" value={String(data.project.special_units_pending)} />
        <Kpi label="הכנסות בסיס מיחידות סטנדרטיות" value={ils(data.project.total_standard_unit_revenue_ils)} emphasize />
      </section>
      ...
      <main className="flex flex-col gap-6 px-6">
        ...
        {currentFamily && <WorkspaceNextSteps family={currentFamily} scenario={scenario} />}
...
function Kpi({ label, value, emphasize }: { label: string; value: string; emphasize?: boolean }) {
  return (
    <div className="rounded-lg border border-slate-300 bg-white p-4">
      <div className={`font-bold text-slate-900 ${emphasize ? "text-2xl" : "text-xl"}`}>{value}</div>
